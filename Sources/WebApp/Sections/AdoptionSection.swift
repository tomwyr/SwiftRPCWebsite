...
struct AdoptionSection {
    ...
    let steps = [
        AdoptionStep(number: "01", title: "Define protocol", copy: "Describe the service with ordinary Swift types. The macro supplies the RPC metadata.", kind: .protocolStep),
        AdoptionStep(number: "02", title: "Implement handler", copy: "Conform to the generated handler contract and keep business logic independent from the transport.", kind: .handlerStep),
        AdoptionStep(number: "03", title: "Call typed client", copy: "Create a client through your server integration and call the service like a local async API.", kind: .clientStep),
    ]

    var body: some View {
        section(.id("adoption"), .class("section adoption"), .custom(name: "aria-labelledby", value: "adoption-title")) {
            div(.class("container")) {
                div(.class("section-intro")) {
                    p(.class("overline")) { "Progressive adoption" }
                    h2(.id("adoption-title")) { "From protocol to production call" }
                    p { "The same service definition moves through each layer without recreating the contract." }
                }
                    ...
                    div(.class("step-copy")) {
                        ...
                        if selectedStep == 2 {
                            div(.class("step-links")) {
                                span(.class("button button-primary button-compact button-disabled"), .custom(name: "aria-label", value: "Read the getting-started guide, coming soon")) {
                                    "Read the guide"
                                    small { "Coming soon" }
                                }
