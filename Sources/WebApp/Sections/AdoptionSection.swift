  ...
  var body: some View {
    PageSection(id: "adoption", titleID: "adoption-title") {
      PageContainer {
        ...
        AdoptionTabs(steps: steps, selectedStep: $selectedStep)
        AdoptionStepPanel(step: steps[selectedStep], selectedStep: selectedStep)
      }
    }
  }
}
...
@View
private struct AdoptionTabs {
  let steps: [AdoptionStep]
  @Binding var selectedStep: Int
...
@View
private struct AdoptionStepPanel {
  ...
  var body: some View {
    article(
      ...
      div {
        p(.class("font-mono text-[13px] text-orange")) { step.number }
        h3(.class("mb-3 text-xl font-bold")) { step.title }
        p(.class("text-[17px] text-secondary")) { step.copy }
        if selectedStep == 2 {
          div(
            .class(
